...
void CustomerDataFormat(void)
{
 //Fill coordinate information to Double buffer
	...
	if(!(ZetVar.bWorkingState==WORKING_STATE_CUSTOMER_NORMAL||ZetVar.bWorkingState==WORKING_STATE_CUSTOMER_INITIAL||ZetVar.bWorkingState==WORKING_STATE_CUSTOMER_GESTURE))
	{
		return;
	}
	#if 0
...
{  
	...
	CustomerVar.bCustomCmd06Cnt = 0;
	CustomerVar.bCustomCmd07Cnt = 0;
	CustomerVar.bCustomCmd18Cnt = 0;
	CustomerVar.bGestureKeyINT = FALSE;
	CustomerVar.bINTtriggerCnt=0xff;
	CustomerVar.bGestureINTtriggerCnt=0;
